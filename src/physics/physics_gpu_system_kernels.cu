...
__global__ void packSparseRigidCommandKernel(const IndexT* logicalIndices,
                                             const uint32_t* physxGpuIndices,
                                             const float* denseCommand,
                                             uint32_t* packedPhysxGpuIndices,
                                             float* packedCommand,
                                             uint32_t count,
                                             uint32_t rigidCount) {
...
uint32_t viewCount(const Sim::GpuArrayView& view) {
    ...
    if (count > static_cast<int64_t>(UINT32_MAX))
        throw std::runtime_error("sparse rigid command index count is too large");
    return static_cast<uint32_t>(count);
}

void validateSparseIndexView(const Sim::GpuArrayView& view, uint32_t rigidCount) {
    if (!view.isCUDA())
        throw std::runtime_error(
            "sparse rigid command indices must be a CUDADevice view");
    if (view.dtype != Sim::SimDType::Int32 &&
        view.dtype != Sim::SimDType::UInt32)
        throw std::runtime_error(
            "sparse rigid command indices must be int32 or uint32");
    if (!view.data)
        throw std::runtime_error(
            "sparse rigid command indices require a non-null data pointer");
    if (view.shape.size() != 1)
        throw std::runtime_error(
            "sparse rigid command indices must have shape [count]");
    if (!view.strides.empty() &&
        (view.strides.size() != 1 || view.strides[0] != 1))
        throw std::runtime_error(
            "sparse rigid command indices must be contiguous");
    if (viewCount(view) > rigidCount)
        throw std::runtime_error(
