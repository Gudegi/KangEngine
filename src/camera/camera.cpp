...
void Camera::updateProjMatrix(const unsigned int scrWidth, const unsigned int scrHeight)
{
    ...
    if (_screenHeight == 0)
    {
        _projMatrix = glm::perspective(glm::radians(_FoV), 1.0f, _nearPlane, _farPlane);
        return;
    }

    _projMatrix = glm::perspective(glm::radians(_FoV), (float)_screenWidth / (float)_screenHeight, _nearPlane, _farPlane);
}
