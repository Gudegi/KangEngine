...
float distanceAttenuation(float distanceToLight, float lightRange) {
    float rangeFalloff = clamp(1.0 - distanceToLight / max(lightRange, 0.0001), 0.0, 1.0);
    return rangeFalloff * rangeFalloff / max(distanceToLight * distanceToLight, 1.0);
}
...
void main() {
    ...
    if(useMetallicMap != 0)
        metallic = clamp(metallic * texture(uMetallicMap, TexCoord).r, 0.0, 1.0);
    if(useRoughnessMap != 0)
        roughness = clamp(roughness * texture(uRoughnessMap, TexCoord).r, 0.04, 1.0);

    if(useAoMap != 0)
        ao *= texture(uAoMap, TexCoord).r;
    ...
    vec3 direct = (1.0 - shadow) *
        evaluatePBRLight(N, V, normalize(lightDir.xyz), lightColor.rgb,
                         albedo, metallic, roughness);

    int pointCount = min(lightCounts.x, MAX_POINT_LIGHTS);
    for(int i = 0; i < pointCount; ++i) {
        vec3 toLight = pointPositionRange[i].xyz - FragPos;
        float distanceToLight = length(toLight);
        vec3 L = toLight / max(distanceToLight, 0.0001);
        vec3 radiance = pointColorIntensity[i].rgb *
            distanceAttenuation(distanceToLight, pointPositionRange[i].w);
        direct += evaluatePBRLight(N, V, L, radiance, albedo, metallic, roughness);
    }

    int spotCount = min(lightCounts.y, MAX_SPOT_LIGHTS);
    for(int i = 0; i < spotCount; ++i) {
        vec3 toLight = spotPositionRange[i].xyz - FragPos;
        float distanceToLight = length(toLight);
        vec3 L = toLight / max(distanceToLight, 0.0001);
        float theta = dot(normalize(-L), normalize(spotDirectionInner[i].xyz));
        float coneFalloff = smoothstep(spotColorOuter[i].w,
                                       spotDirectionInner[i].w, theta);
        vec3 radiance = spotColorOuter[i].rgb * coneFalloff *
            distanceAttenuation(distanceToLight, spotPositionRange[i].w);
        direct += evaluatePBRLight(N, V, L, radiance, albedo, metallic, roughness);
    }

    vec3 ambientColor = ambient.rgb * albedo * ao;
    vec3 color = ambientColor + direct + emissive;
