...
float ShadowCalculation() {
    float shadowOrthoHalfSize = shadowInfo.x;
    float shadowMapWH = shadowInfo.y;
    int pcfSamples = clamp(int(shadowInfo.z + 0.5), 1, 16);
    ...
    vec4 lightSpacePos =
        lightSpaceMatrix * vec4(WorldPos + normal * normalBias, 1.0);
    ...
    for(int i = 0; i < 16; i++) {
        ...
        float pcfDepth =
            texture(shadowMap, projCoords.xy + offset * shadowRadius * texelSize)
                .r;
