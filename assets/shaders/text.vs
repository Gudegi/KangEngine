#version 410 core
layout(location = 0) in vec2 aPosition;
layout(location = 1) in vec2 aUv;
layout(location = 2) in vec3 iOrigin;
layout(location = 3) in vec2 iOffset;
layout(location = 4) in vec2 iSize;
layout(location = 5) in vec4 iUvRect;
layout(location = 6) in vec4 iColor;

layout(std140) uniform cameraUBO {
    mat4 view;
    mat4 projection;
};

uniform vec2 uViewportSize;
uniform bool uScreenSpace;

out vec2 vUv;
out vec4 vColor;

void main() {
    if (uScreenSpace) {
        vec2 pixelPosition = iOrigin.xy + iOffset + aPosition * iSize;
        vec2 ndc = vec2(
            pixelPosition.x * 2.0 / uViewportSize.x - 1.0,
            1.0 - pixelPosition.y * 2.0 / uViewportSize.y
        );
        gl_Position = vec4(ndc, 0.0, 1.0);
        vUv = vec2(
            mix(iUvRect.x, iUvRect.z, aUv.x),
            mix(iUvRect.w, iUvRect.y, aUv.y)
        );
        vColor = iColor;
        return;
    }
    vec4 clipOrigin = projection * view * vec4(iOrigin, 1.0);
    vec2 pixelPosition = iOffset + aPosition * iSize;
    vec2 clipOffset = pixelPosition * 2.0 / uViewportSize * clipOrigin.w;
    gl_Position = clipOrigin + vec4(clipOffset, 0.0, 0.0);
    vUv = mix(iUvRect.xy, iUvRect.zw, aUv);
    vColor = iColor;
}
