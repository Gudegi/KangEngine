...
float grid(vec2 uv, float lineWidth) {
    ...
    vec2 lines =
        1.0 - smoothstep(drawWidth - lineAA, drawWidth + lineAA, gridUV);
...
void main() {
    ...
    vec3 lit = ambient.rgb * albedo +
               (1.0 - debugShadow) * (diffuse + specular);
