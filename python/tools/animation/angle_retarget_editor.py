"""Interactive angle-retarget configuration editor."""
...
from kangengine.animation.retarget.io import load_retarget_dataset_motion
...
def _retarget_config_output_path(path: Path) -> Path:
    if path.name.endswith("_angle_retarget.json"):
        return path
    if path.suffix.lower() == ".json":
        return path.with_name(f"{path.stem}_angle_retarget.json")
    return path.with_name(f"{path.name}_angle_retarget.json")
...
class EditableSkeleton:
    ...
    def __init__(
        ...
        self.joint_color = np.asarray(joint_color, dtype=np.float32)
        ...
        for joint, name in enumerate(tree.node_names()):
            ...
            handle.set_base_color(joint_color)
        ...
        if asset_path.suffix.lower() == ".fbx":
            ...
            try:
                config = visual.SkeletalVisualConfig(
                    ...
                    joint_color=ke.Vec4(*joint_color),
                    bone_radius=0.018,
                    joint_radius=0.0,
                    segments=8,
                    visible=True,
    ...
    def sync_debug(self, show_axes: bool) -> None:
        ...
        if starts:
            ...
            bone_path = f"{self.root_path}/bones"
            ...
            bone_prim = self.app.scene.get_prim_at_path(bone_path)
            if bone_prim is not None:
                bone_prim.set_pickable(False)
...
class RetargetEditor(ke.App):
    def __init__(
        ...
        *,
        ...
        self.config_path_text = (
            "retarget_config_angle_retarget.json"
        ...
        self.source_panel = RetargetSourcePanel(
            ...
            dialog_key="angle_source_motion",
        )
        ...
        self.ascend_person = ascend_person
        self.ascend_pose_key = ascend_pose_key
        self.ascend_translation_key = ascend_translation_key
        self.ascend_fps = ascend_fps
    ...
    def setup(self) -> None:
        ...
        self.config: animation.AngleRetargetConfig | None = None
        ...
        self.source_preview: RetargetMotionPreview | None = None
        self.target_preview: RetargetMotionPreview | None = None
    ...
    def source_coordinates(self):
        return self.source_settings.coordinate_system

    @source_coordinates.setter
    def source_coordinates(self, value) -> None:
        self.source_settings.coordinate_system = ke.animation.CoordinateSystem(value)

    @property
    def source_translation_unit_scale(self) -> float:
        return self.source_settings.translation_unit_scale

    @source_translation_unit_scale.setter
    def source_translation_unit_scale(self, value: float) -> None:
        self.source_settings.translation_unit_scale = float(value)

    @property
    def source_has_armature_joint(self) -> bool:
        return self.source_settings.has_armature_joint

    @source_has_armature_joint.setter
    def source_has_armature_joint(self, value: bool) -> None:
        self.source_settings.has_armature_joint = bool(value)
    ...
    def _render_coordinate_selector(self, label: str, attribute: str) -> None:
        ...
        for index, (value, text) in enumerate(
            (
                (ke.utils.CoordinateSystem.Y_UP_Z_FORWARD, "Y-up Z-fwd"),
                (
                    ke.utils.CoordinateSystem.Y_UP_NEG_Z_FORWARD,
                    "Y-up -Z-fwd",
                ),
                (ke.utils.CoordinateSystem.Z_UP_X_FORWARD, "Z-up X-fwd"),
            )
        ):
    ...
    def _load_source(self) -> None:
        try:
            ...
            dataset_type = animation.datasets.detect_dataset_type(path)
            dataset_skeleton = None
            dataset_bind_root = (0.0, 0.0, 0.0)
            if dataset_type is not None:
                config_path = self._path(self.config_path_text)
                config = animation.AngleRetargetConfig.load(config_path)
                profile = config.source_profile
                if profile is None or profile.reference_skeleton is None:
                    raise ValueError("config requires a source reference skeleton")
                source_path = Path(profile.reference_skeleton).expanduser()
                if not source_path.is_absolute():
                    source_path = config_path.parent / source_path
                source_path = source_path.resolve()
                loaded = _load_skeleton(source_path, _default_scale(source_path))
                source_coordinates = ke.utils.CoordinateSystem(
                    profile.coordinate_system
                )
                dataset_skeleton = animation.convert_skeleton_coordinates(
                    loaded,
                    source=source_coordinates,
                    target=ke.utils.CoordinateSystem.Y_UP_Z_FORWARD,
                )
                dataset_bind_root = config.source_bind_root
                self.source_coordinates = source_coordinates
                self.source_has_armature_joint = profile.has_armature_joint
                self.source_translation_unit_scale = profile.translation_unit_scale
            if dataset_type is not None:
                assert dataset_skeleton is not None
                self.source_motion = load_retarget_dataset_motion(
                    path,
                    dataset_skeleton,
                    target_coordinate_system=(
                        ke.utils.CoordinateSystem.Y_UP_Z_FORWARD
                    ),
                    root_translation_offset=dataset_bind_root,
                    ascend_person=self.ascend_person,
                    ascend_pose_key=self.ascend_pose_key,
                    ascend_translation_key=self.ascend_translation_key,
                    ascend_fps=self.ascend_fps,
                )
            else:
                profile = animation.MotionSourceProfile(
                    name=path.stem,
                    coordinate_system=self.source_coordinates.value,
                    translation_unit_scale=self.source_translation_unit_scale,
                    has_armature_joint=self.source_has_armature_joint,
                    reference_skeleton=path,
                )
                self.source_motion = RetargetSourceLoader(
                    profile,
                    ke.utils.CoordinateSystem.Y_UP_Z_FORWARD,
                ).load(path)
    ...
    def _check_loaded(self) -> None:
        if self.source_edit is None or self.target_edit is None:
            return
        self.config = animation.AngleRetargetConfig(
            joint_map={},
            output_coordinate_system=self.output_coordinates.value,
            source_profile=animation.MotionSourceProfile(
                name=Path(self.source_path_text).stem,
                coordinate_system=self.source_coordinates.value,
                translation_unit_scale=self.source_translation_unit_scale,
                has_armature_joint=self.source_has_armature_joint,
                reference_skeleton=Path(self.source_path_text),
            ),
            target_profile=animation.AngleTargetProfile(
                name=Path(self.target_path_text).stem,
                skeleton=Path(self.target_path_text),
                coordinate_system=self.target_coordinates.value,
            ),
        )
    ...
    def _sync_config(self) -> animation.AngleRetargetConfig:
        ...
        if self.config.source_profile is None or self.config.target_profile is None:
            raise RuntimeError("retarget profiles are not initialized")
        self.config.source_profile = replace(
            self.config.source_profile,
            coordinate_system=self.source_coordinates.value,
            translation_unit_scale=self.source_translation_unit_scale,
            has_armature_joint=self.source_has_armature_joint,
        )
        self.config.target_profile = replace(
            self.config.target_profile,
            coordinate_system=self.target_coordinates.value,
        )
    ...
    def _load_config(self) -> None:
        try:
            ...
            if target_profile is None:
                raise ValueError("config requires a target profile")
            ...
            source_path = resolve_asset_path(str(source_profile.reference_skeleton))
            target_path = resolve_asset_path(str(target_profile.skeleton))
            ...
            current_source_path = self._path(self.source_path_text)
            preserve_ascend_motion = (
                current_source_path.suffix.lower() in {".npz", ".npy", ".pkl"}
                and current_source_path.is_file()
            )
            self.source_path_text = str(
                current_source_path if preserve_ascend_motion else source_path
            )
            self.target_path_text = str(target_path)
            self.source_coordinates = ke.utils.CoordinateSystem(
                source_profile.coordinate_system
            )
            self.target_coordinates = ke.utils.CoordinateSystem(
                target_profile.coordinate_system
            )
            self.output_coordinates = ke.utils.CoordinateSystem(
                config.output_coordinate_system
            )
            self.source_has_armature_joint = source_profile.has_armature_joint
            self.source_translation_unit_scale = source_profile.translation_unit_scale
            self._load_source()
            self._load_target()
            if self.source_edit is None or self.target_edit is None:
            ...
            self._set_edit_mode(EditMode.INSPECT)
            self.status = (
                f"Config loaded with source "
                f"{Path(self.source_path_text).name} "
                f"(skeleton {source_path.name}) and "
    ...
    def _export_config(self) -> None:
        try:
            if not self.config_path_text.strip():
                self.config_path_text = "retarget_config_angle_retarget.json"
    ...
    def _compute_retarget(self, frame_limit: int | None) -> None:
        ...
        try:
            ...
            self.result_motion = animation.AngleRetargeter(
    ...
    def _preview_source(self) -> None:
        ...
        source = animation.scale_skeleton_motion(
            source, self.source_edit.uniform_root_scale()
        )
    ...
    def _ensure_source_preview_visual(self, source: animation.SkeletonMotion) -> None:
        if self.source_preview is None:
            self.source_preview = RetargetMotionPreview(
                self,
            ...
            self.source_preview.set_motion(source)
    ...
    def _ensure_preview_visuals(self, source: animation.SkeletonMotion) -> None:
        ...
        self.preview_target_motion = animation.scale_skeleton_motion(
            self.result_motion, self.target_edit.uniform_root_scale()
        )
        self._ensure_source_preview_visual(source)
        if self.target_preview is None:
            self.target_preview = RetargetMotionPreview(
                self,
                self.materials.common,
                "/RetargetPreview/B",
                self.preview_target_motion,
                config=visual.SkeletalVisualConfig(
                    bone_color=ke.Vec4(1.0, 0.55, 0.2, 1.0), show_joints=True
                ),
            )
        else:
            self.target_preview.set_motion(self.preview_target_motion)
    ...
    def _apply_preview_time(self, time: float) -> None:
        ...
        self.source_preview.offset = (source_x_offset, 0.0, 0.0)
        self.source_preview.apply_frame(source_frame)
        if self.source_preview_skin is not None:
        ...
        self.target_preview.offset = (1.5, 0.0, 0.0)
        self.target_preview.apply_frame(frame)
        if self.target_preview_skin is not None:
    ...
    def _render_load(self) -> None:
        source_action = self.source_panel.render(
            self.dialogs,
            label="Source motion A",
            loaded=self.source_edit is not None,
            current_scale=(
                None
                if self.source_edit is None
                else self.source_edit.uniform_root_scale()
            ),
        ...
        if source_action.scale_factor is not None:
            self._multiply_scale("A", source_action.scale_factor)
        self._render_coordinate_selector("Target B input", "target_coordinates")
        _, self.target_path_text = retarget_path_input(
        ...
        imgui.same_line()
        if _button("Load##target_skeleton", "primary"):
            self._load_target()
        ...
        if _button("Browse##retarget_config"):
            self.dialogs.open_file(
                "retarget_config",
                "Select Retarget Config",
                "JSON files{.json}",
                str(retarget_assets_directory()),
            )
        imgui.same_line()
        if _button("Load##retarget_config", "primary"):
            self._load_config()
        imgui.same_line()
    ...
    def _render_file_dialogs(self) -> None:
        for key, attribute in (
            ...
            selected_path = self.dialogs.selected(key)
    ...
    def _render_workflow(self) -> None:
        if self.showing_result:
            view_mode = (
                RetargetViewMode.PREVIEW_SOURCE
                if self.preview_source_only
                else RetargetViewMode.PREVIEW_RESULT
            )
        else:
            view_mode = RetargetViewMode.EDIT_MAPPING
        selected_mode = retarget_view_mode_buttons(
            view_mode,
            source_available=self.source_motion is not None,
            result_available=self.result_motion is not None,
        )
        if selected_mode is not view_mode:
            if selected_mode is RetargetViewMode.EDIT_MAPPING:
                self._set_edit_mode(EditMode.MAPPING)
            elif selected_mode is RetargetViewMode.PREVIEW_SOURCE:
        ...
        imgui.text_disabled(
            "Suggested workflow: Facing -> Scale -> Bind Pose -> Edit Mapping -> "
            "Preview Source/Result -> Export"
        )
        for mode, label in (
...
def main() -> None:
    ...
    app = RetargetEditor(
        ...
        args.config,
        ascend_person=args.ascend_person,
        ascend_pose_key=args.ascend_pose_key,
        ascend_translation_key=args.ascend_translation_key,
        ascend_fps=args.ascend_fps,
    )
